% --- Load the dataset ---
data = readtable('new_dishwasher.csv');
dishwasher=data.Dishwasher;
time = data.Time; % Datetime vector

% --- Set the thresholds and minimum duration to consider a switch-on ---
threshold_dishwasher = 10;  % Threshold for the dishwasher
min_duration_samples = 225;   % Corresponds to 24 seconds (3 samples of 8s each)
...
switch_ons_dishwasher = count_switch_ons(dishwasher, threshold_dishwasher, min_duration_samples, weekdays, hours_of_day);

% --- Calculate the average switch-ons for the dishwasher ---
average_switch_ons_dishwasher = zeros(7, 24);
for day = 1:7
    valid_hours = switch_ons_dishwasher(day,:) > 0; % Only consider hours with activations
    if any(valid_hours)
        average_switch_ons_dishwasher(day, valid_hours) = switch_ons_dishwasher(day, valid_hours) / sum(valid_hours);
    end
end
...
% --- Display the average results with bar plots for the first 4 days for Dishwasher ---
...
for day = 1:4
    ...
    bar(0:23, average_switch_ons_dishwasher(day, :), 'FaceColor', 'b');
    title(['Average Dishwasher Switch-ons - ', day_names{day}]);
    xlabel('Hour of the day');
    ylabel('Average Number of Switch-ons');
    ylim([0, max(max(average_switch_ons_dishwasher))]); % Set uniform y-axis limit across all plots
end


% --- Display the average results with bar plots for the remaining 3 days for Dishwasher ---
figure('Units', 'normalized', 'Position', [0, 0, 0.5, 1.5]); % Set figure with taller aspect ratio
for day = 5:7
